...
func setValidConfigEnv(t *testing.T) {
	...
	t.Setenv("ALLOW_RESET", "false")
	t.Setenv("AUTO_RESET_CREDITS", "false")
...
func TestLoadConfigAutoResetCredits(t *testing.T) {
	testCases := []struct {
		name      string
		value     string
		want      bool
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			setValidConfigEnv(t)
			t.Setenv("AUTO_RESET_CREDITS", testCase.value)
			...
			if cfg.autoResetCredits != testCase.want {
				t.Fatalf("autoResetCredits = %t, want %t", cfg.autoResetCredits, testCase.want)
			}
		})
	}
}
